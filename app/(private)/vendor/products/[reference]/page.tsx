...
export default function ProductPage() {
    ...
    if (isError || !product) {
        return (
            <div className="min-h-screen bg-background p-8 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-muted-foreground mb-4">The product you are looking for does not exist or you do not have permission to view it.</p>
                <button
                    onClick={() => router.back()}
    ...
    const currency = product.shop_details?.currency || vendor?.shop?.currency || "KES";
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span
                                className={`px-2 py-0.5 rounded-full text-[10px] uppercase tracking-tighter ${product.is_active
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="bg-white border border-secondary/30 rounded-sm p-6 shadow-sm">
                            <h3 className="font-serif text-lg mb-4 flex items-center text-foreground">
                            ...
                            {product.images && product.images.length > 0 ? (
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {product.images.map((img, idx) => (
                                        <div key={idx} className="aspect-square rounded-sm overflow-hidden border border-secondary/20 relative group">
                                            <img
                                                src={img.image}
                                    ...
                                    <tbody className="divide-y divide-secondary/10">
                                        {product.variants && product.variants.length > 0 ? (
                                            product.variants.map((variant) => (
                                                <tr key={variant.id} className="hover:bg-secondary/5 transition-colors">
                                                    <td className="px-6 py-4 text-sm font-medium text-foreground">
                                                        {Object.entries(variant.attributes).map(([key, value]) => (
                                                            <span key={key} className="inline-block mr-2 bg-secondary/10 px-2 py-1 rounded text-xs text-foreground/80">
                                                                {key}: <span className="font-semibold">{String(value)}</span>
                                                            </span>
                                                        ))}
                                                        {Object.keys(variant.attributes).length === 0 && <span className="text-muted-foreground text-xs italic">Default Variant</span>}
                                                    </td>
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Description */}
                        <div className="bg-white border border-secondary/30 rounded-sm p-6 shadow-sm">
                            <h4 className="text-xs uppercase tracking-widest text-muted-foreground mb-3 font-medium">Description</h4>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {product.sub_category && product.sub_category.length > 0 ? (
                                        product.sub_category.map((sub, idx) => (
                                            <span key={idx} className="bg-primary/5 text-primary border border-primary/20 px-2 py-1 rounded text-xs font-medium">
                                                {sub.name}
                                            </span>
                                        ))
                                    ) : (
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {product.tags && product.tags.length > 0 ? (
                                        product.tags.map((tag, idx) => (
                                            <span key={idx} className="bg-secondary/10 text-foreground/70 px-2 py-1 rounded text-xs">
                                                #{tag}
                                            </span>
                                        ))
                                    ) : (
