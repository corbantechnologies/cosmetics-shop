...
export default function UpdateSubCategory({ reference, onSuccess }: { reference: string, onSuccess?: () => void }) {
    const authHeaders = useAxiosAuth();
    const [loading, setLoading] = useState(false);

    const { data: subCategory, isLoading: subCategoryLoading, isError } = useFetchSubCategory(reference);
    ...
    if (subCategoryLoading) return <div className="p-4 text-center">Loading details...</div>;
    if (isError) return <div className="p-4 text-center text-red-500">Error loading subcategory.</div>;
